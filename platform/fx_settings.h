...
/* fx_settings.h - the effect toggles.
 *
 * These are the effect toggles (CONTRACT.md section 4: "every required
 ...
 * Presentation code reads g_effects every frame, so a toggle takes
...
const char *fx_settings_name(int i);             /* "bloom", "particles", ... */
uint8_t    *fx_settings_ptr(EffectSettings *s, int i);
