...
/* Session seed from the OS. The engine's rng_os_seed() replaces this at merge. */
static uint64_t os_seed(void)
{
    uint64_t s = 0;
    if (getrandom(&s, sizeof s, 0) == (ssize_t)sizeof s) return s;
    struct timespec ts;
    clock_gettime(CLOCK_REALTIME, &ts);
    return ((uint64_t)ts.tv_sec * 1000000007ULL) ^ (uint64_t)ts.tv_nsec ^ ((uint64_t)getpid() << 32);
}
...
int main(int argc, char **argv)
{
    ...
    uint64_t seed = o.have_seed ? o.seed : os_seed();
    ...
    while (!g_stop) {
        ...
        /* 2. fixed-step logic */
        int ticks = o.lockstep ? 1 : step_advance(&step, frame ? snap_to_vsync(t0 - t_prev) : TICK_NS);
        t_prev = t0;
