...
/* engine/input.h - logical inputs, one frame per 60 Hz tick. The platform
   maps keys, pads and touch onto these; games only ever see InputFrame. */
enum {
...
#define BTN_COUNT 19
#define BTN_ALL   ((1u << BTN_COUNT) - 1u)

/* The HOLD button for card i (0..4). */
#define BTN_HOLD_N(i) (BTN_HOLD1 << (i))
...
/* Fills f->pressed from f->down and the previous tick's down mask, so every
   producer of frames (platform, attract driver, scripts) derives edges the
   same way. */
static inline void input_set_pressed(InputFrame *f, uint32_t prev_down)
{
  f->pressed = f->down & ~prev_down;
}
